...
function createFiltersHTMLCode(array, listDiv) {
  ...
  for (i = 0; i < array.length; i++) {
    listDiv.innerHTML += `<div id="filter-${array[i].type}-${i}" class="filters--selection background--${array[i].type}"><div class="container"><span class="filters--selection--text">${array[i].name}</span></div><button class="btn" onclick="removeFilter(${i}, '${array[i].type}')"><i class="fa-solid fa-xmark text-white"></i></button></div>`;
  }
}
